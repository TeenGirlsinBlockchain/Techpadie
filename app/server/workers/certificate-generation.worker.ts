...
const WORKER_ID = `cert-worker-${process.pid}`;

let isShuttingDown = false;

async function main() {
  console.log(`🎓 Techpadie Certificate Worker started (ID: ${WORKER_ID})`);
  ...
  console.log('👋 Certificate Worker shutting down gracefully...');
  process.exit(0);
}
...
main().catch((err) => {
  console.error('💀 Certificate Worker crashed:', err);
  process.exit(1);
});
