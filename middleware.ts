    ...
    if (origin) {
      try {
        ...
        if (originUrl.host !== host) {
          return NextResponse.json(
            { success: false, error: 'CSRF verification failed: Origin host mismatch' },
            { status: 403 }
          );
        }
      } catch {
        return NextResponse.json(
          { success: false, error: 'CSRF verification failed: Invalid Origin' },
          { status: 400 }
        );
      }
    } else if (referer) {
      try {
        ...
        if (refererUrl.host !== host) {
          return NextResponse.json(
            { success: false, error: 'CSRF verification failed: Referer host mismatch' },
            { status: 403 }
          );
        }
      } catch {
        return NextResponse.json(
          { success: false, error: 'CSRF verification failed: Invalid Referer' },
          { status: 400 }
        );
      }
    }
